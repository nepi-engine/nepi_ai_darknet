...
class Yolov3AIF(object):
    TYPICAL_LOAD_TIME_PER_MB = 1.5

    ai_node_dict = dict()

    def __init__(self, aif_dict,launch_namespace, all_namespace, models_lib_path):
    ...
    def getModelsDict(self):
        ...
        if os.path.exists(self.models_folder_path) == False:
            ...
            for f in self.cfg_files:
                ...
                if success == False:
                    nepi_msg.printMsgWarn(self.log_name + ": File does not appear to be a valid A/I model param file: " + f + "... not adding this model")
                    continue
                ...
                if ("framework" not in cfg_dict_keys):
                    nepi_msg.printMsgWarn(self.log_name + ": Framework not specified in model yaml file: " + f + "... not adding this model")
                    continue
                #nepi_msg.printMsgWarn(self.log_name + ": Imported model key names: " + str(cfg_dict_keys))
                if ("config_file" not in cfg_dict_keys):
                    nepi_msg.printMsgWarn(self.log_name + ": File does not specified in model yaml file: " + f + "... not adding this model")
                    continue
                #nepi_msg.printMsgWarn(self.log_name + ": Imported model key names: " + str(cfg_dict_keys))
                if ("weight_file" not in cfg_dict_keys):
                    nepi_msg.printMsgWarn(self.log_name + ": File does not specify a weight file: " + f + "... not adding this model")
                    continue
                #nepi_msg.printMsgWarn(self.log_name + ": Imported model key names: " + str(cfg_dict_keys))
                ...
                #nepi_msg.printMsgWarn(self.log_name + ": Checking that model config file exists: " + config_file_path + " for model name " + model_name)
                config_file = cfg_dict[model_key]["config_file"]["name"]
                config_file_path = os.path.join(self.models_folder_path,config_file)
                if not os.path.exists(config_file_path):
                    nepi_msg.printMsgWarn(self.log_name + ": Model " + model_name + " specifies non-existent config file " + config_file_path + "... not adding this model")
                    continue
    ...
    def loadModel(self, model_dict):
        ...
        nepi_msg.printMsgInfo(self.log_name + ": Launching Yolov3 AI node " + model_name + " with commands: " + str(launch_cmd_line))
    ...
    def killModel(self,model_name):
        if model_name in self.ai_node_dict.keys():
            ...
            nepi_msg.printMsgInfo(self.log_name + ": Killing Yolov3 AI node: " + model_name)
