...
'pkg_name': 'nepi_aif_yolov3', 
'if_file_name': 'aif_yolov3_if.py', 
'if_path_name': '/opt/nepi/ros/share/nepi_aifs', 
'if_module_name': 'aif_yolov3_if', 
'if_class_name': 'Yolov3AIF', 
'models_folder_name': 'yolov3', 
'model_prefix': 'ai_yolov3_', 
'launch_pkg_name': 'nepi_ai_yolov3',
'launch_file_name': 'yolov3_ros.launch', 
'node_file_name': 'nepi_ai_yolov3_node.py',  
'active': True
}

TEST_LAUNCH_NAMESPACE = "/nepi/yolov3_test"
TEST_all_namespace = "/nepi/ai_detector_mgr"
TEST_MODELS_LIB_PATH = "/mnt/nepi_storage/ai_models/"



class Yolov3AIF(object):
    TYPICAL_LOAD_TIME_PER_MB = 3.5

    ai_node_dict = dict()
    def __init__(self, ai_dict,launch_namespace, all_namespace, models_lib_path):
      ...
      nepi_msg.printMsgInfo("Yolov3 models path: " + self.models_folder_path)
    ...
    def getModelsDict(self):
        models_dict = dict()
        # Try to obtain the path to Yolov3 models from the system_mgr
        nepi_msg.printMsgInfo("ai_yolov3_if: Looking for model files in folder: " + self.models_folder_path)
        # Grab the list of all existing yolov3 cfg files
        if os.path.exists(self.models_folder_path) == False:
            nepi_msg.printMsgInfo("ai_yolov3_if: Failed to find models folder: " + self.models_folder_path)
            return models_dict
        else:
            self.cfg_files = glob.glob(os.path.join(self.models_folder_path,'*.yaml'))
            nepi_msg.printMsgInfo("ai_yolov3_if: Found network config files: " + str(self.cfg_files))
            # Remove the ros.yaml file -- that one doesn't represent a selectable trained neural net
            for f in self.cfg_files:
                cfg_dict = dict()
                success = False
                try:
                    #nepi_msg.printMsgWarn("ai_yolov3_if: Opening yaml file: " + f) 
                    yaml_stream = open(f, 'r')
                    success = True
                    #nepi_msg.printMsgWarn("ai_yolov3_if: Opened yaml file: " + f) 
                except Exception as e:
                    nepi_msg.printMsgWarn("ai_yolov3_if: Failed to open yaml file: " + str(e))
                if success:
                    try:
                        # Validate that it is a proper config file and gather weights file size info for load-time estimates
                        #nepi_msg.printMsgWarn("ai_yolov3_if: Loading yaml data from file: " + f) 
                        cfg_dict = yaml.load(yaml_stream)  
                        model_keys = list(cfg_dict.keys())
                        model_key = model_keys[0]
                        #nepi_msg.printMsgWarn("ai_yolov3_if: Loaded yaml data from file: " + f) 
                    except Exception as e:
                        nepi_msg.printMsgWarn("ai_yolov3_if: Failed load yaml data: " + str(e)) 
                        success = False 
                try: 
                    #nepi_msg.printMsgWarn("ai_yolov3_if: Closing yaml data stream for file: " + f) 
                    yaml_stream.close()
                except Exception as e:
                    nepi_msg.printMsgWarn("ai_yolov3_if: Failed close yaml file: " + str(e))
                
                if success == False:
                    nepi_msg.printMsgWarn("ai_yolov3_if: File does not appear to be a valid A/I model param file: " + f + "... not adding this model")
                    continue
                #nepi_msg.printMsgWarn("ai_yolov3_if: Import success: " + str(success) + " with cfg_dict " + str(cfg_dict))
                ...
                if ("framework" not in cfg_dict_keys):
                    nepi_msg.printMsgWarn("ai_yolov3_if: Framework not specified in model yaml file: " + f + "... not adding this model")
                    continue
                #nepi_msg.printMsgWarn("ai_yolov3_if: Imported model key names: " + str(cfg_dict_keys))
                if ("config_file" not in cfg_dict_keys):
                    nepi_msg.printMsgWarn("ai_yolov3_if: File does not specified in model yaml file: " + f + "... not adding this model")
                    continue
                #nepi_msg.printMsgWarn("ai_yolov3_if: Imported model key names: " + str(cfg_dict_keys))
                if ("weight_file" not in cfg_dict_keys):
                    nepi_msg.printMsgWarn("ai_yolov3_if: File does not specify a weight file: " + f + "... not adding this model")
                    continue
                #nepi_msg.printMsgWarn("ai_yolov3_if: Imported model key names: " + str(cfg_dict_keys))
                ...
                if framework != 'yolov3':
                    nepi_msg.printMsgWarn("ai_yolov3_if: Model " + model_name + " not a yolov3 model" + framework + "... not adding this model")
                    continue


                #nepi_msg.printMsgWarn("ai_yolov3_if: Checking that model config file exists: " + config_file_path + " for model name " + model_name)
                config_file = cfg_dict[model_key]["config_file"]["name"]
                config_file_path = os.path.join(self.models_folder_path,config_file)
                if not os.path.exists(config_file_path):
                    nepi_msg.printMsgWarn("ai_yolov3_if: Model " + model_name + " specifies non-existent config file " + config_file_path + "... not adding this model")
                    continue

                weight_file = cfg_dict[model_key]["weight_file"]["name"]
                weight_file_path = os.path.join(self.models_folder_path,weight_file)
                #nepi_msg.printMsgWarn("ai_yolov3_if: Checking that model weights file exists: " + weight_file_path + " for model name " + model_name)
                if not os.path.exists(weight_file_path):
                    nepi_msg.printMsgWarn("ai_yolov3_if: Model " + model_name + " specifies non-existent weights file " + weight_file_path + "... not adding this model")
                    continue
                model_type = cfg_dict[model_key]['type']['name']
                if model_type not in self.node_file_dict.keys():
                    nepi_msg.printMsgWarn("ai_yolov3_if: Model " + model_name + " specifies non-supported model type " + model_type + "... not adding this model")
                    continue
                else:
                ...
                nepi_msg.printMsgInfo("ai_yolov3_if: Model dict create for model : " + model_name)
    ...
    def loadModel(self, model_dict):
        ...
        # Build Yolov3 new model_name launch command
        launch_cmd_line = [
            ...
            "config_file_path:=" + os.path.join(model_dict['model_path'],model_dict['config_file']),
            "weight_file_path:=" + os.path.join(model_dict['model_path'],model_dict['weight_file'])
        ]
        nepi_msg.printMsgInfo("ai_yolov3_if: Launching Yolov3 AI node " + model_name + " with commands: " + str(launch_cmd_line))
    ...
    def killModel(self,model_name):
        if model_name in self.ai_node_dict.keys():
            ...
            nepi_msg.printMsgInfo("ai_yolov3_if: Killing Yolov3 AI node: " + model_name)
...
if __name__ == '__main__':
    node_name = "ai_yolov3_test"
    while nepi_ros.check_for_node(node_name):
        nepi_msg.printMsgInfo("ai_yolov3_if: Trying to kill running node: " + node_name)
        nepi_ros.kill_node(node_name)
        nepi_ros.sleep(2,10)
    Yolov3AIF(TEST_AI_DICT,TEST_LAUNCH_NAMESPACE,TEST_all_namespace,TEST_MODELS_LIB_PATH)
